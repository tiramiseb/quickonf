...
func (c *checker) recipeDoc(toks tokens) (next tokens, newVars map[string]string) {
	if len(toks) != 2 {
...
func (c *checker) recipeVarDoc(toks tokens) (next tokens, newVars map[string]string) {
	if len(toks) >= 2 {
