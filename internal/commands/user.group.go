...
var userGroup = Command{
	...
	func(args []string) (result []string, msg string, apply *Apply, status Status) {
		...
		if exists {
			willMessage = fmt.Sprintf("Will create group %s and make %s a member of it", groupname, username)
			needMessage = fmt.Sprintf("Need to create group %s and make %s a member of it", groupname, username)
		} else {
			willMessage = fmt.Sprintf("Will make %s a member of group %s", username, groupname)
